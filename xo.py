...
RE_TWO_DIGITS = re.compile(r'(\d+)(\D+)?(\d+)?')
...
DEFAULT_RC['rgb_to_short'] = {
    v: k
    for k, v in DEFAULT_RC['short_to_rgb'].items()
}
...
class LineEditor(urwid.Edit):
    ...
    def __init__(self, edit_text="", lexer=None, main_display=None,
                 smart_home=True, tabsize=None, **kwargs):
...
class DequeEditor(urwid.Edit):
    """
      An editor that uses values from a deque or list.
...
class LineWalker(urwid.ListWalker):
    ...
    def get_pos(self, w):
        ...
        if pos_guess < llen and w is lines[pos_guess]:
            return pos_guess
        for uppos, dnpos in zip_longest(
                range(pos_guess + 1, llen),
                range(pos_guess - 1, -1, -1)
        ):
    ...
    def seek_match(self, q):
        ...
        while m is None and last_pos != curr_pos:
            ...
            m = q.search(
                w.get_edit_text(),
                w.edit_pos + 1 if curr_pos == orig_pos else 0
            )
        if m is None:
    ...
    def get_tokens(self, w, window=None):
        """
          Computes the tokens for a widget, but first tries to look them up
            from a cache on the class.
        """
...
class MainDisplay(object):
    ...
    def replace_match(self):
        """
          Finds, jumps, and substitues to the next match for the current
            query & replacement.
        """
        if not self.queries:
            stat = "no re   "
    ...
    def reset_status(self, *args, status="xo      ", **kwargs):
        ncol, _ = self.loop.screen.get_cols_rows()
        ft = self.status_text
        ft[1][0] = status
...
def main(args=None):
    ...
    parser.add_argument('path', nargs='?', help=(
        "path to file, may include colon separated "
